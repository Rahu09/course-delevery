...
function CoursePage() {
  ...
  useEffect(() => {
    if (content.content) setChapters(content.content.chapters);
  }, [content]);
  ...
  const handlePurchase = () => {
    if (baseUrl === "/api/admin") {
      ...
      axios.post(`${baseUrl}/courses/${id}`).then(() => {
        setBought(true);
